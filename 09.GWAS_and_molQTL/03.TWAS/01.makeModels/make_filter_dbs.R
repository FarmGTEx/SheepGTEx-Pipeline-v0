...
conn <- dbConnect(drv = driver, './dbs/sheepgtex_models.db')
...
# Sample_info Table ----
sample_info <- data.frame(n_samples = n_samples, population = 'sheep') # Provide the population info
...
unfiltered_db <- './dbs/sheepgtex_models.db'
filtered_db <- './dbs/sheepgtex_models_filtered_signif.db'
driver <- dbDriver("SQLite")
