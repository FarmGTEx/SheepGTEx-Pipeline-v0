...
citation: https://doi.org/10.1093/molbev/msaa103
"""
...
@click.option('--smid', help='input sample name')
...
def indivstat(samfile, smid, autosites, chrxsites, chrysites, outprefix, savedp):
    alnfile = pysam.AlignmentFile(samfile)
    autodp = samdepth(samfile, autosites)
    print(f'auto sites: {autodp.shape[0]}')
    # Use the mode as the standardized denominator
    mode, count = stats.mode(autodp)
    mode = mode[0]
    count = count[0]
    print(f'{smid} auto mode: {mode}. count: {count}')
    if mode > 1:
        xdp = samdepth(samfile, chrxsites)
        print(f'{smid} x sites: {xdp.shape[0]}')
        xmedian = np.median(xdp)
        print(f'{smid} X median: {xmedian}')

        ydp = samdepth(samfile, chrysites)
        print(f'{smid} y sites: {ydp.shape[0]}')
        ymedian = np.median(ydp)
        print(f'{smid} Y median: {ymedian}')
        ...
        print(f'{smid} auto depth low than 1, use mean')
        print(f'{smid} auto mean: {automean}')
        xdp = samdepth(samfile, chrxsites)
        print(f'{smid} x sites: {xdp.shape[0]}')
        xmean = np.mean(xdp)
        print(f'{smid} X mean: {xmean}')
        ydp = samdepth(samfile, chrysites)
        print(f'{smid} y sites: {ydp.shape[0]}')
        ymean = np.mean(ydp)
        print(f'{smid} Y mean: {ymean}')
...
@main.command('popcluster', short_help='cluster the sex mode')
