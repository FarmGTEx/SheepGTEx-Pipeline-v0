...
# 设置工作目录
setwd('"C:/Users/au783873/OneDrive - 西北农林科技大学/备份/博后/GTEx/results/03.molQTL/eQTL/tissue_sharing"')
...
#lfsr_all <- readRDS(file.path("03.top_pair_eeQTL", "lfsr_m.s_2.RDS"))
pm <- readRDS("./pm_m.s.RDS")
#pm_all <- readRDS(file.path("03.top_pair_eeQTL", "pm_m.s_2.RDS"))
...
#cor_matrix <- cor(pm_sig_corr, method = "kendall")
#dist_matrix <- as.dist(1 - cor_matrix)
#hc <- hclust(dist_matrix, method = "complete")
#od <- hc$order
#pm_sig_corr <- pm_sig_corr[od, od]
#nm <- rownames(pm_sig_corr)
#row_hc <- hclust(as.dist(1 - cor(pm_sig_corr, method = "kendall")), method = "complete")
#column_hc <- hclust(as.dist(1 - cor(t(pm_sig_corr), method = "kendall")), method = "complete")
