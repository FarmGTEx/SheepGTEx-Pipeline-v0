...
input_file = sys.argv[4] # tpm expression file: genes x samples
y_train_file = sys.argv[5] # sample file for training containing two columns: sample\tGender
...
pred_file = "{0}.F{1}.{2}.pred.tsv".format(feature_method, num_features, class_model)
#--- read gene expression data
data = pd.read_csv(input_file, delimiter='\t', header=0, index_col=0).T
# log2(expr + 1) transfer 
data = np.log2(data + 1)
Y_train = pd.read_csv(y_train_file, delimiter='\t', header=0, index_col=0)
train = pd.merge(data, Y_train, left_index=True, right_index=True)
...
X_test = data.loc[~data.index.isin(Y_train.index)]
...
# save results
pred_results = pd.DataFrame(np.vstack((X_test.index, y_pred)).T, columns=['sample', 'Gender'])
pred_results.to_csv(pred_file, sep="\t", index=None)
