...
file1 = ARGS[1] # nominal file of eQTL
file2 = ARGS[2] # nominal file of other QTL
genefile = ARGS[3] # list of significant genes in at least one QTL
tissue = ARGS[4] # tissue name
chrom = ARGS[5] # chromosome name
outfile = ARGS[6] # output file of coloc
groupfile = ARGS[7] # phenotype_groups of QTLs in file2
...
if (file.exists(groupfile)) { 
  group = fread(groupfile, header = T)
  nominal2 = inner_join(nominal2, group, by = "phenotype_id")
  df = inner_join(nominal1, nominal2, by = c("phenotype_id" = "gene_id", "variant_id"))
} else {
